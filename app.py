...
def load_content(genre_id=None):
    try:
        ...
        # Discover works better for genre filtering
        ...
        if res.get("results"):
            ...
            for item in res["results"]:
                ...
                p_names = ", ".join([p['provider_name'] for p in providers]) if providers else "Rent/Buy only"
...
if st.session_state.liked:
    ...
    # SMALL CLEAR BUTTON
    if st.button("Clear Matches", key="clear_btn"):
        st.session_state.liked = []
        st.rerun()
